$(document).ready(function () {
    ...
    let addBtn = $('#addCategory');

    addBtn.on('click', function () {
        showModal();
    })

    let showModal = function (type = null, id = null) {
    ...
    let table = $('#datatable-category').DataTable({
        ...
        createdRow: function (row, data) {
            let actions = '<button style="margin-right: 5px" type="button" class="mb-1 btn btn-warning btn-pill edit-category"><i class=" mdi mdi-pencil mr-1"></i></button>';
        ...
        if ($('form#form-add-category').valid()) {
            ...
            $.ajax({
                ...
                error: function (res) {
                    $('#editCategory').html('Save');
                    $('#editCategory').attr("disabled", false);
                    let msgs = res.responseJSON.errors.type ? res.responseJSON.errors.type : '';
                    Swal.fire(
                        'Update category',
